...
const PRODUCTS: Product[] = [
  {
    ...
    description: 'A professional-grade football tactics board engineered for coaches, analysts, and tactical thinkers. Draw formations, animate movements, and build complex strategic scenarios with a precision-built toolset designed from the ground up for the modern game.',
    status: 'live',
    route: '/apps/pitchpro',
    specs: [
  ...
  },
  {
    ...
    description: 'Advanced performance metrics dashboard for sports teams. Data ingestion, visualization, and AI-driven pattern recognition for post-match analysis.',
    status: 'development',
  },
  {
    id: 'simflow',
    ...
    description: 'Industrial process simulation and optimization tool. Discrete-event modeling meets intuitive visual programming. Built for industrial engineers by an industrial engineer.',
    status: 'concept',
  },
];

const ROADMAP: RoadmapItem[] = [
  { id: 'r1', period: '2024 — Q4',    title: 'PitchPro v16 — Football Tactics Board',      description: 'Full-featured football tactics board. Player management, formation drawing, animation system, export functionality. 16 iterations to production quality.', status: 'done'    },
  { id: 'r2', period: '2025 — Q1–Q2', title: 'ie Dynamics Portal v1.0',                    description: 'Main brand portal. Product showcase, documentation hub, brand identity. Next.js 15, TypeScript, Tailwind CSS, PWA-ready. Google Play compliant.',          status: 'active'  },
  { id: 'r3', period: '2025 — Q2–Q3', title: 'AnalytIQ — Sports Performance Dashboard',    description: 'Data-driven performance analytics for teams and individual athletes. Video tagging, KPI dashboards, comparative analysis across seasons.',                   status: 'planned' },
  { id: 'r4', period: '2025 — Q4',    title: 'SimFlow — Process Simulation Engine',         description: 'Visual discrete-event simulation for industrial processes. Bottleneck analysis, optimization recommendations.',                                              status: 'planned' },
  { id: 'r5', period: '2026',          title: 'ORsuite — Operations Research Toolkit',      description: 'Linear programming, scheduling, supply chain optimization — all in a browser-native interface. Engineering-grade computation, consumer-grade UX.',            status: 'planned' },
];
...
function StatusDot({ label }: { label?: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      <span style={{ display: 'inline-block', width: '6px', height: '6px', borderRadius: '50%', background: '#00e87a', boxShadow: '0 0 8px #00e87a', animation: 'pulseDot 2s ease-in-out infinite' }} />
      {label && (
...
function SectionLabel({ text, index }: { text: string; index: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '56px' }}>
      ...
      <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '0.25em', textTransform: 'uppercase', color: '#00e87a' }}>{text}</span>
      <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: '#4a5568', marginLeft: 'auto' }}>{index}</span>
    </div>
  );
}
...
function MiniPitch() {
  const homePos: [number,number][] = [[30,110],[90,65],[90,95],[90,125],[90,155],[145,80],[145,110],[145,140],[215,70],[215,110],[215,150]];
  const awayPos: [number,number][] = [[290,110],[235,65],[235,95],[235,125],[235,155],[185,80],[185,110],[185,140],[175,110]];
  return (
    <div style={{ width: '100%', height: '100%', background: '#0d2a1a', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
      ...
      <svg viewBox="0 0 320 220" fill="none" style={{ width: '80%', height: '75%', opacity: 0.6 }}>
        ...
        {homePos.map(([cx,cy],i) => (
          <circle key={`h${i}`} cx={cx} cy={cy} r="7" fill={i===0?'rgba(0,232,122,0.8)':'rgba(0,232,122,0.6)'} stroke={i===0?'#00e87a':'none'} strokeWidth="1" />
        ))}
        {awayPos.map(([cx,cy],i) => (
...
export default function HomePage() {
  ...
  useEffect(() => {
    const onMove  = (e: MouseEvent) => { mouseRef.current = { x: e.clientX, y: e.clientY }; setCursorPos({ x: e.clientX, y: e.clientY }); setCursorVisible(true); };
    const onLeave = () => setCursorVisible(false);
    const onOver  = (e: MouseEvent) => { setCursorHover(!!(e.target as Element)?.closest('a, button, [role="button"]')); };
    ...
    const animate = () => {
      ...
      if (ringRef.current) { ringRef.current.style.left = `${ringPosRef.current.x}px`; ringRef.current.style.top = `${ringPosRef.current.y}px`; }
      rafRef.current = requestAnimationFrame(animate);
    };
    rafRef.current = requestAnimationFrame(animate);
    return () => { document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseleave', onLeave); document.removeEventListener('mouseover', onOver); cancelAnimationFrame(rafRef.current); };
  }, []);
  ...
  return (
    <>
      <style>{`
        ...
        :root { --font-head:'Syne',sans-serif; --font-mono:'Space Mono',monospace; --font-body:'DM Sans',sans-serif; }
        *,*::before,*::after { box-sizing:border-box; margin:0; padding:0; }
        html { scroll-behavior:smooth; }
        body { background:#040507; color:#e8edf4; font-family:var(--font-body); font-weight:300; overflow-x:hidden; cursor:none; -webkit-font-smoothing:antialiased; }
        @media (hover:none) { body { cursor:auto; } .ie-cursor,.ie-cursor-ring { display:none !important; } }
        ::-webkit-scrollbar { width:3px; } ::-webkit-scrollbar-track { background:#040507; } ::-webkit-scrollbar-thumb { background:#00e87a; }
        ::selection { background:rgba(0,232,122,0.2); }
        @keyframes pulseDot { 0%,100% { opacity:1; box-shadow:0 0 8px #00e87a; } 50% { opacity:0.5; box-shadow:0 0 4px #00e87a; } }
        @keyframes fadeUp { from { opacity:0; transform:translateY(30px); } to { opacity:1; transform:translateY(0); } }
        @keyframes ticker { from { transform:translateX(0); } to { transform:translateX(-50%); } }
        .anim-in { opacity:0; animation:fadeUp 0.7s cubic-bezier(0.16,1,0.3,1) forwards; }
        .d1{animation-delay:.10s} .d2{animation-delay:.20s} .d3{animation-delay:.35s} .d4{animation-delay:.50s}
        .reveal { opacity:0; transform:translateY(24px); transition:opacity 0.7s cubic-bezier(0.16,1,0.3,1),transform 0.7s cubic-bezier(0.16,1,0.3,1); }
        .revealed { opacity:1; transform:translateY(0); }
        .ghost-text { -webkit-text-stroke:1px rgba(255,255,255,0.15); color:transparent; }
        .product-card { transition:background 0.3s; } .product-card:hover { background:#121820 !important; }
        .tech-item:hover { background:#0d1219 !important; border-color:rgba(0,232,122,0.15) !important; }
        .nav-link::after { content:''; position:absolute; bottom:-4px; left:0; right:0; height:1px; background:#00e87a; transform:scaleX(0); transition:transform 0.2s; }
        .nav-link:hover::after,.nav-link.active::after { transform:scaleX(1); }
        .nav-link:hover { color:#e8edf4 !important; }
        .btn-primary-hover:hover { transform:translateY(-2px); box-shadow:0 16px 48px rgba(0,232,122,0.3); }
        .btn-secondary-hover:hover { border-color:rgba(0,232,122,0.4) !important; background:rgba(0,232,122,0.05) !important; }
        .ticker-track { animation:ticker 25s linear infinite; }
        .section-base { position:relative; z-index:2; padding:120px 48px; }
      ...
      {/* Cursor */}
      <div className="ie-cursor" style={{ position:'fixed', zIndex:9999, pointerEvents:'none', width:cursorHover?'14px':'8px', height:cursorHover?'14px':'8px', background:'#00e87a', borderRadius:'50%', left:cursorPos.x, top:cursorPos.y, transform:'translate(-50%,-50%)', opacity:cursorVisible?1:0, transition:'width 0.2s,height 0.2s,opacity 0.2s' }} />
      <div ref={ringRef} className="ie-cursor-ring" style={{ position:'fixed', zIndex:9998, pointerEvents:'none', width:cursorHover?'48px':'32px', height:cursorHover?'48px':'32px', border:'1px solid rgba(0,232,122,0.4)', borderRadius:'50%', transform:'translate(-50%,-50%)', opacity:cursorVisible?1:0, transition:'width 0.2s,height 0.2s,opacity 0.2s' }} />

      {/* Grid */}
      <div style={{ position:'fixed', inset:0, zIndex:0, pointerEvents:'none', backgroundImage:'linear-gradient(rgba(0,232,122,0.025) 1px,transparent 1px),linear-gradient(90deg,rgba(0,232,122,0.025) 1px,transparent 1px)', backgroundSize:'60px 60px' }} />

      {/* Noise */}
      <div style={{ position:'fixed', inset:0, zIndex:1000, pointerEvents:'none', opacity:0.4, backgroundImage:`url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='0.03'/%3E%3C/svg%3E")` }} />

      {/* ── NAVBAR ── */}
      <nav style={{ position:'fixed', top:0, left:0, right:0, zIndex:100, height:'72px', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 48px', borderBottom:'1px solid rgba(255,255,255,0.06)', background:scrolled?'rgba(4,5,7,0.92)':'rgba(4,5,7,0.70)', backdropFilter:'blur(20px)', WebkitBackdropFilter:'blur(20px)', transition:'background 0.3s' }}>
        <a href="/" style={{ display:'flex', alignItems:'center', gap:'14px', textDecoration:'none' }}>
          <LogoMark size={36} />
          <div style={{ display:'flex', flexDirection:'column', lineHeight:1 }}>
            <span style={{ fontFamily:'var(--font-head)', fontWeight:800, fontSize:'18px', color:'#00e87a', letterSpacing:'-0.02em' }}>ie Dynamics</span>
            <span style={{ fontFamily:'var(--font-mono)', fontSize:'9px', letterSpacing:'0.2em', textTransform:'uppercase', color:'#4a5568', marginTop:'2px' }}>Engineering Intelligence</span>
          </div>
        </a>
        ...
        <div className="nav-links" style={{ display:'flex', alignItems:'center', gap:'40px' }}>
          {NAV_LINKS.map((l) => {
            ...
            return (
              <a key={l.href} href={l.href} className={`nav-link${isActive?' active':''}`} style={{ fontFamily:'var(--font-mono)', fontSize:'11px', letterSpacing:'0.12em', textTransform:'uppercase', textDecoration:'none', position:'relative', color:isActive?'#e8edf4':'#4a5568', transition:'color 0.2s' }}>
                {l.label}
              </a>
            );
          })}
      ...
      <section id="hero" style={{ position:'relative', minHeight:'100vh', display:'flex', alignItems:'center', padding:'152px 48px 80px', overflow:'hidden', zIndex:2 }}>
        ...
        <div style={{ position:'relative', zIndex:2, maxWidth:'900px' }}>
          ...
          <div className="anim-in d3" style={{ display:'flex', alignItems:'center', gap:'20px', flexWrap:'wrap' }}>
            <Link href="#products" style={btnPrimary} className="btn-primary-hover">Explore Products <ArrowRight /></Link>
            <Link href="#about" style={btnSecondary} className="btn-secondary-hover">Our Approach</Link>
          </div>
        </div>
        ...
        <div className="hero-stats anim-in d4" style={{ position:'absolute', right:'48px', bottom:'80px', display:'flex', flexDirection:'column', gap:'24px', zIndex:2 }}>
          {[{num:'01',accent:'.',label:'Live Product'},{num:'v',accent:'16',label:'Current Build'},{num:'∞',accent:'',label:'Possibilities'}].map((s) => (
            <div key={s.label} style={{ textAlign:'right' }}>
              <div style={{ fontFamily:'var(--font-head)', fontWeight:800, fontSize:'36px', lineHeight:1, letterSpacing:'-0.02em' }}>{s.num}<span style={{ color:'#00e87a' }}>{s.accent}</span></div>
              <div style={{ fontFamily:'var(--font-mono)', fontSize:'9px', letterSpacing:'0.2em', textTransform:'uppercase', color:'#4a5568', marginTop:'4px' }}>{s.label}</div>
            </div>
          ))}
        </div>
      ...
      <div style={{ borderTop:'1px solid rgba(255,255,255,0.06)', borderBottom:'1px solid rgba(255,255,255,0.06)', padding:'12px 0', overflow:'hidden', background:'#080b0f', position:'relative', zIndex:2 }}>
        <div className="ticker-track" style={{ display:'flex', whiteSpace:'nowrap' }}>
          {[...TICKER_ITEMS,...TICKER_ITEMS].map((item,i) => (
            <span key={i} style={{ display:'inline-flex', alignItems:'center', padding:'0 40px', fontFamily:'var(--font-mono)', fontSize:'10px', letterSpacing:'0.15em', textTransform:'uppercase', color:'#4a5568' }}>{item}</span>
          ))}
        </div>
      </div>
      ...
      <section id="products" className="section-base" style={{ background:'#080b0f' }}>
        ...
        <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', flexWrap:'wrap', gap:'24px', marginBottom:'64px' }}>
          <h2 style={{ fontFamily:'var(--font-head)', fontWeight:800, fontSize:'clamp(36px,5vw,64px)', lineHeight:'0.95', letterSpacing:'-0.02em' }}>Our<br /><span className="ghost-text">Instruments</span></h2>
          <p style={{ maxWidth:'360px', fontSize:'14px', lineHeight:'1.7', color:'rgba(232,237,244,0.5)' }}>Precision-engineered analytical tools. Built for professionals who demand accuracy and depth from their digital workflows.</p>
        </div>
          ...
          {PRODUCTS.map((p,idx) => {
            ...
            if (p.featured) {
              return (
                <div key={p.id} className="product-card reveal featured-grid" style={{ gridColumn:'1 / -1', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'64px', alignItems:'center', padding:'48px', background:'#0d1219', border:'1px solid rgba(255,255,255,0.06)', borderTop:'2px solid #00e87a', position:'relative', overflow:'hidden' }}>
                  <div>
                    <span style={{ display:'inline-flex', alignItems:'center', gap:'6px', fontFamily:'var(--font-mono)', fontSize:'9px', letterSpacing:'0.2em', textTransform:'uppercase', padding:'4px 10px', borderRadius:'1px', marginBottom:'24px', background:cfg.bg, color:cfg.color, border:`1px solid ${cfg.border}` }}>{cfg.label}</span>
                  ...
                  <div style={{ aspectRatio:'4/3', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'2px', overflow:'hidden' }}><MiniPitch /></div>
            ...
            return (
              <div key={p.id} className="product-card reveal" style={{ padding:'48px', background:'#0d1219', border:'1px solid rgba(255,255,255,0.06)', position:'relative', overflow:'hidden' }}>
                ...
                <p style={{ fontFamily:'var(--font-mono)', fontSize:'11px', color:'#4a5568', letterSpacing:'0.1em', marginBottom:'20px' }}>{p.version} — {p.tagline}</p>
                <p style={{ fontSize:'14px', lineHeight:'1.7', color:'rgba(232,237,244,0.55)', marginBottom:'32px' }}>{p.description}</p>
                <a href="#roadmap" style={{ display:'inline-flex', alignItems:'center', gap:'8px', fontFamily:'var(--font-head)', fontWeight:700, fontSize:'12px', letterSpacing:'0.1em', textTransform:'uppercase', color:accentColor, textDecoration:'none' }}>View Roadmap <ArrowRight size={16} /></a>
      ...
      <section id="tech" className="section-base" style={{ background:'#040507', borderTop:'1px solid rgba(255,255,255,0.06)', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
        ...
        <h2 style={{ fontFamily:'var(--font-head)', fontWeight:800, fontSize:'clamp(36px,5vw,64px)', lineHeight:'0.95', letterSpacing:'-0.02em', marginBottom:'64px' }}>Built with<br /><span className="ghost-text">Precision</span></h2>
      ...
      <section id="about" className="section-base" style={{ background:'#0d1219' }}>
        ...
        <div className="manifesto-grid" style={{ display:'grid', gridTemplateColumns:'1fr 2fr', gap:'80px', alignItems:'start' }}>
          ...
          <div>
            <h2 style={{ fontFamily:'var(--font-head)', fontWeight:800, fontSize:'clamp(32px,4vw,56px)', lineHeight:'1.0', letterSpacing:'-0.02em', marginBottom:'40px' }}>Where Engineering<br />Meets Product.</h2>
      ...
      <section id="roadmap" className="section-base" style={{ background:'#040507' }}>
        ...
        <h2 style={{ fontFamily:'var(--font-head)', fontWeight:800, fontSize:'clamp(36px,5vw,64px)', lineHeight:'0.95', letterSpacing:'-0.02em', marginBottom:'64px' }}>What&apos;s<br /><span className="ghost-text">Coming</span></h2>
          ...
          {ROADMAP.map((item) => {
            ...
            return (
              <div key={item.id} className="reveal" style={{ position:'relative', paddingBottom:'56px' }}>
                ...
                <span style={{ display:'inline-block', fontFamily:'var(--font-mono)', fontSize:'9px', letterSpacing:'0.15em', textTransform:'uppercase', padding:'3px 8px', borderRadius:'1px', marginBottom:'12px', background:cfg.bg, color:cfg.color }}>{cfg.label}</span>
      ...
      <section id="contact" className="section-base" style={{ background:'#080b0f', borderTop:'1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ maxWidth:'800px', margin:'0 auto', textAlign:'center' }}>
          <h2 style={{ fontFamily:'var(--font-head)', fontWeight:800, fontSize:'clamp(40px,6vw,80px)', letterSpacing:'-0.03em', lineHeight:'0.95', marginBottom:'24px' }}>Let&apos;s Build<br />Something <span style={{ color:'#00e87a' }}>Real.</span></h2>
          <p style={{ fontSize:'15px', color:'rgba(232,237,244,0.5)', marginBottom:'48px', lineHeight:'1.7' }}>Interested in collaborating, investing, or just want to follow the journey? ie Dynamics is an open project — every product ships publicly.</p>
          <div style={{ display:'flex', gap:'16px', justifyContent:'center', flexWrap:'wrap' }}>
            <a href={`mailto:${BRAND.email}`} style={btnPrimary} className="btn-primary-hover">Get in Touch <ArrowRight /></a>
            <a href="#products" style={btnSecondary} className="btn-secondary-hover">Explore Products</a>
          </div>
            ...
            {[{type:'Email',value:BRAND.email},{type:'Status',value:BRAND.status},{type:'Location',value:BRAND.location}].map((c) => (
              <div key={c.type} className="reveal" style={{ padding:'24px', border:'1px solid rgba(255,255,255,0.06)', background:'#0d1219' }}>
                ...
                <p style={{ fontFamily:'var(--font-head)', fontWeight:600, fontSize:'15px' }}>
                  {c.type==='Email' ? <a href={`mailto:${c.value}`} style={{ color:'#e8edf4', textDecoration:'none' }}>{c.value}</a> : c.value}
                </p>
              </div>
            ))}
          </div>
